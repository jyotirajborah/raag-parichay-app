    ...
    function renderDNAGrid() {
        ...
        const dnaRows = {
            alpha: {
                ...
                color: '#4fc3f7',
                shrutis: [
                    { index: 1, symbol: 'S', name: 'Shadja' },
                    { index: 4, symbol: 'R1', name: 'Shuddha Rishabh' },
                    { index: 8, symbol: 'G1', name: 'Shuddha Gandhar' },
                    { index: 10, symbol: 'm1', name: 'Shuddha Madhyam' },
                    { index: 14, symbol: 'P', name: 'Pancham' },
                    { index: 17, symbol: 'D1', name: 'Shuddha Dhaivat' },
                    { index: 21, symbol: 'N1', name: 'Shuddha Nishad' }
                ],
                description: 'Pure harmonic ratios (5/4, 4/3, 3/2, 5/3, 15/8). These notes form perfect consonances and are used in pentatonic raags like Bhupali.'
            },
            beta: {
                name: 'Beta Row (Pythagorean)',
                color: '#ffd54f',
                shrutis: [
                    { index: 1, symbol: 'S', name: 'Shadja' },
                    { index: 5, symbol: 'R2', name: 'Teevra Shuddha Rishabh' },
                    { index: 9, symbol: 'G2', name: 'Teevra Shuddha Gandhar' },
                    { index: 11, symbol: 'm2', name: 'Ek Shruti Madhyam' },
                    { index: 14, symbol: 'P', name: 'Pancham' },
                    { index: 18, symbol: 'D2', name: 'Teevra Shuddha Dhaivat' },
                    { index: 22, symbol: 'N2', name: 'Teevra Shuddha Nishad' }
                ],
                description: 'Pythagorean ratios (9/8, 81/64, 27/16, 243/128). Sharper and more energetic than Alpha row. Used in bright, dynamic raags.'
            },
            gamma: {
                name: 'Gamma Row (Komal/Flat)',
                color: '#81c784',
                shrutis: [
                    { index: 1, symbol: 'S', name: 'Shadja' },
                    { index: 2, symbol: 'r1', name: 'Ati Komal Rishabh' },
                    { index: 3, symbol: 'r2', name: 'Komal Rishabh' },
                    { index: 6, symbol: 'g1', name: 'Ati Komal Gandhar' },
                    { index: 7, symbol: 'g2', name: 'Komal Gandhar' },
                    { index: 12, symbol: 'M1', name: 'Teevra Madhyam' },
                    { index: 13, symbol: 'M2', name: 'Teevratama Madhyam' },
                    { index: 14, symbol: 'P', name: 'Pancham' },
                    { index: 15, symbol: 'd1', name: 'Ati Komal Dhaivat' },
                    { index: 16, symbol: 'd2', name: 'Komal Dhaivat' },
                    { index: 19, symbol: 'n1', name: 'Ati Komal Nishad' },
                    { index: 20, symbol: 'n2', name: 'Komal Nishad' }
                ],
                description: 'Flat variants and sharp Madhyams. Creates tension, devotion, and melancholy. Essential for raags like Bhairav, Todi, Darbari.'
            }
        };
        
        let html = `
            <div class="dna-intro-card">
                <h2>The DNA of Raags</h2>
                <p>Dr. Vidyadhar Oke's research reveals that every raag is "cut" from a universal harmonic grid. This DNA Grid shows three fundamental rows of shrutis, each with distinct harmonic properties. Raags are formed by selecting specific paths through this grid.</p>
                <div class="dna-key-concepts">
                    <div class="concept-box">
                        <h4>Horizontal Movement (+9)</h4>
                        <p>Moving horizontally within a row represents the Madhyam relationship (perfect fourth)</p>
                    </div>
                    <div class="concept-box">
                        <h4>Vertical Movement (+13)</h4>
                        <p>Moving between rows represents the Pancham relationship (perfect fifth)</p>
                    </div>
                    <div class="concept-box">
                        <h4>Row Consistency</h4>
                        <p>Staying within one row (Alpha or Beta) ensures stable consonance. Mixing rows creates tension.</p>
                    </div>
                </div>
            </div>
        `;
        ...
        // Render each row
        Object.keys(dnaRows).forEach((rowKey, rowIndex) => {
            const row = dnaRows[rowKey];
            html += `
                <div class="dna-row-container" style="animation-delay: ${rowIndex * 0.2}s">
                    <div class="dna-row-header" style="border-left-color: ${row.color}">
                        <h3 style="color: ${row.color}">${row.name}</h3>
                        <p>${row.description}</p>
                    </div>
                    <div class="dna-row-grid">
            `;
            
            row.shrutis.forEach((shruti, idx) => {
                const nextShruti = row.shrutis[idx + 1];
                const hasConnection = nextShruti && (nextShruti.index - shruti.index === 9 || 
                                                     nextShruti.index - shruti.index === 13 ||
                                                     nextShruti.index - shruti.index === 3 ||
                                                     nextShruti.index - shruti.index === 4 ||
                                                     nextShruti.index - shruti.index === 7);
                
                html += `
                    <div class="dna-node-wrapper">
                        <div class="dna-node" 
                             data-index="${shruti.index}" 
                             data-symbol="${shruti.symbol}"
                             style="border-color: ${row.color}">
                            <div class="dna-node-index">#${shruti.index}</div>
                            <div class="dna-node-symbol">${shruti.symbol}</div>
                            <div class="dna-node-name">${shruti.name}</div>
                        </div>
                        ${hasConnection ? `<div class="dna-connection" style="background: ${row.color}">
                            <span class="connection-label">+${nextShruti.index - shruti.index}</span>
                        </div>` : ''}
                    </div>
                `;
            });
        ...
        // Add example raag paths
        html += `
            <div class="dna-examples-section">
                <h3>Example Raag Paths</h3>
                <div class="dna-examples-grid">
                    <div class="dna-example-card">
                        <h4>Bhupali (Pentatonic)</h4>
                        <div class="path-display">S → R1 → G1 → P → D1</div>
                        <p class="path-indices">Indices: 1 → 4 → 8 → 14 → 17</p>
                        <p class="path-description">Pure Alpha Row path. All notes use just intonation ratios for maximum consonance.</p>
                    </div>
                    <div class="dna-example-card">
                        <h4>Yaman (Evening)</h4>
                        <div class="path-display">S → R2 → G1 → M1 → P → D2 → N1</div>
                        <p class="path-indices">Indices: 1 → 5 → 8 → 12 → 14 → 18 → 21</p>
                        <p class="path-description">Mixed path using Beta (R2, D2) and Gamma (M1) with Alpha (G1, N1). Creates evening tension.</p>
                    </div>
                    <div class="dna-example-card">
                        <h4>Malkauns (Pentatonic Minor)</h4>
                        <div class="path-display">S → g1 → m1 → d1 → n1</div>
                        <p class="path-indices">Indices: 1 → 6 → 10 → 15 → 19</p>
                        <p class="path-description">Pure Gamma Row path with komal notes. Creates dark, mysterious atmosphere.</p>
                    </div>
                    <div class="dna-example-card">
                        <h4>Bhairav (Morning)</h4>
                        <div class="path-display">S → r2 → G1 → m1 → P → d2 → N1</div>
                        <p class="path-indices">Indices: 1 → 3 → 8 → 10 → 14 → 16 → 21</p>
                        <p class="path-description">Gamma komal (r2, d2) with Alpha shuddha (G1, N1). Sacred morning contrast.</p>
                    </div>
                    <div class="dna-example-card">
                        <h4>Todi (Devotional)</h4>
                        <div class="path-display">S → r1 → g1 → M1 → P → d1 → N1</div>
                        <p class="path-indices">Indices: 1 → 2 → 6 → 12 → 14 → 15 → 21</p>
                        <p class="path-description">Extreme Gamma komals (r1, g1, d1) with sharp M1. Maximum devotional tension.</p>
                    </div>
                    <div class="dna-example-card">
                        <h4>Marwa (Dramatic)</h4>
                        <div class="path-display">S → r1 → G2 → M1 → D2 → N1</div>
                        <p class="path-indices">Indices: 1 → 2 → 9 → 12 → 18 → 21</p>
                        <p class="path-description">Lowest r1 with highest G2 - extreme contrast creates royal, dramatic character.</p>
                    </div>
                </div>
            </div>
        `;
        ...
        function playDNAFrequency(frequency, duration = 1000) {
